...
| `TECHNICALLY_PAUSED` | 2 | A protocol technical pause (global or for this machine) blocks activation, renewal and the Solana `subscription` phase. The SDK reads both pause flags before any approval, so nothing was spent. Wait for the pause to clear, then preview again. `machine subscription renew` names the code; on `activate` a CLI release without a row for it reports `ACTIVATION_FAILED`. |
