...
| `RPC_FAILED` with `MachineSubscription.fullMode() could not be read` (the SDK's own code is `READ_FAILED`) | 2 | The installed SDK reads a contract function the chain no longer has (mainnet renamed it on 2026-09-15). Run `pip install -U peaq-os-cli peaq-os-sdk`; if the newest release still fails, the fix is not out yet. |
